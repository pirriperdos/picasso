...
public class RequestCreatorTest {
  ...
  public void getOnMainCrashes() throws Exception {
    try {
      new RequestCreator(picasso, new Request.Builder(URI_1)).get();
  ...
  @Test public void loadWithShutdownCrashes() throws Exception {
    ...
    try {
      new RequestCreator(picasso, new Request.Builder(URI_1)).fetch();
  ...
  @Test public void getReturnsNullIfNullUriAndResourceId() throws Exception {
    ...
    new Thread(new Runnable() {
      @Override public void run() {
        try {
          result[0] = new RequestCreator(picasso, new Request.Builder(URI_1)).get();
        } catch (IOException e) {
          fail(e.getMessage());
        } finally {
          latch.countDown();
  ...
  @Test public void fetchWithFitThrows() throws Exception {
    try {
      new RequestCreator(picasso, new Request.Builder(URI_1)).fit().fetch();
  ...
  public void intoTargetWithNullThrows() throws Exception {
    try {
      new RequestCreator(picasso, new Request.Builder(URI_1)).into((Target) null);
  ...
  public void intoTargetWithFitThrows() throws Exception {
    try {
      ...
      new RequestCreator(picasso, new Request.Builder(URI_1)).fit().into(target);
  ...
  public void intoTargetWithNullUriAndResourceIdSkipsAndCancels() throws Exception {
    ...
    new RequestCreator(picasso, new Request.Builder(URI_1)).placeholder(placeHolderDrawable).into(target);
  ...
  public void intoTargetWithQuickMemoryCacheCheckDoesNotSubmit() throws Exception {
    ...
    Target target = mockTarget();
    new RequestCreator(picasso, new Request.Builder(URI_1)).into(target);
  ...
  public void intoTargetAndSkipMemoryCacheDoesNotCheckMemoryCache() throws Exception {
    Target target = mockTarget();
    new RequestCreator(picasso, new Request.Builder(URI_1)).skipMemoryCache().into(target);
  ...
  public void intoImageViewWithNullThrows() throws Exception {
    try {
      new RequestCreator(picasso, new Request.Builder(URI_1)).into((ImageView) null);
  ...
  public void intoImageViewWithQuickMemoryCacheCheckDoesNotSubmit() throws Exception {
    Picasso picasso =
        spy(new Picasso(Robolectric.application, mock(Dispatcher.class), Cache.NONE, null, IDENTITY,
            mock(Stats.class), true, null));
    doReturn(BITMAP_1).when(picasso).quickMemoryCacheCheck(URI_KEY_1);
    ImageView target = mockImageViewTarget();
    Callback callback = mockCallback();
    new RequestCreator(picasso, new Request.Builder(URI_1)).into(target, callback);
  ...
  public void intoImageViewSetsPlaceholderWithResourceId() throws Exception {
    Picasso picasso =
        spy(new Picasso(Robolectric.application, mock(Dispatcher.class), Cache.NONE, null, IDENTITY,
            mock(Stats.class), true, null));
    ImageView target = mockImageViewTarget();
    new RequestCreator(picasso, new Request.Builder(URI_1)).placeholder(R.drawable.picture_frame).into(target);
  ...
  public void intoImageViewWithFitAndResizeThrows() throws Exception {
    try {
      ImageView target = mockImageViewTarget();
      new RequestCreator(picasso, new Request.Builder(URI_1)).fit().resize(10, 10).into(target);
